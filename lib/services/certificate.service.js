...
export async function getAllCertificates(query = {}) {
  const allowedSort = ['studentName', 'courseName', 'graduationYear', 'issuedDate', 'createdAt', 'status'];
  const { page, limit, offset, search, sort, order } = parsePagination(query, allowedSort, 'createdAt');

  const conditions = [];
  if (query.status) conditions.push(eq(certificates.status, query.status));
  if (query.courseId) conditions.push(eq(certificates.courseId, query.courseId));
  if (query.graduationYear) conditions.push(eq(certificates.graduationYear, parseInt(query.graduationYear)));
...
/**
 * getCertificateById — retrieve single certificate by primary key.
 */
